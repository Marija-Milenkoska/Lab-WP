...
public class ChefServiceImpl implements ChefService {
    ...
    public Chef findById(Long id) {
        return chefRepository.findById(id)
                .orElseThrow(()-> new RuntimeException("Chef with id " + id + " was not found"));
    }
    ...
    public Chef addDishToChef(Long chefId, String dishId) {
        ...
        if(dish == null){
            throw new RuntimeException("Dish with id " + dishId + " was not found");
        }

        if(chef.getDishes() == null){
            chef.setDishes(new ArrayList<>());
        }
        if(chef.getDishes().stream().noneMatch(x -> x.getDishId().equals(dish.getDishId()))){
